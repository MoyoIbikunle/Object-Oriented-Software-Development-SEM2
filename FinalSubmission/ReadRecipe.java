...
// PURPOSE: This class retrieves recipe data from the database.
//          It can display all recipes, search for recipes by name or ingredient,
//          adjust ingredient quantities and nutrition values based on servings,
//          and retrieve full recipe details for editing.
...
public class ReadRecipe {
    ...
    // This method retrieves all recipes from the recipes table.
    // It returns each recipe's ID, name, default servings,
    // preparation time, and cooking time as a formatted string.
    public String getAllRecipes() throws SQLException {
        ...
        try {
            ...
            //loops through each recipe and details to output string
            while (resultSet.next()) {
                output = output +
                        "ID: " + resultSet.getInt("recipe_id") +
                        " | Name: " + resultSet.getString("recipe_name") +
                        " | Servings: " + resultSet.getInt("default_servings") +
                        " | Prep Time: " + resultSet.getInt("prep_time") + "mins" +
                        " | Cook Time: " + resultSet.getInt("cook_time") + "mins\n";
            }
    ...
    // This method searches for recipes using a search term.
    // The search checks both recipe names and ingredient names.
    // It returns detailed recipe information including instructions,
    ...
    //query to get all ingredients linked to specifc recipe
    ...
    //query to get nutrition values for a specific recipe
    ...
    try {
        ...
        //loop through each matching recipe
        while (resultSet.next()) {
            ...
            // get nutrition details and display them
            output += "\nNUTRITION:\n";
        ...
        //if no matches were found, return a message instead
...
// This method adjusts a recipe's ingredient quantities, nutrition values,
    // and estimated times based on a new desired number of servings.
    // It returns the adjusted recipe details as a formatted string
public String adjustServings(int recipeId, int desiredServings) throws SQLException {
    ...
    try {
        ...
        // scaling ratio based on number of servings
        double ratio = (double) desiredServings / defaultServings;

        // estimates adjusted times
        ...
        // Step 2 - get ingredients  and scale them
        String ingredientSql = "SELECT ingredients.name, recipe_ingredients.quantity, recipe_ingredients.unit " +
                               "FROM recipe_ingredients " +
    ...
    // This method retrieves all recipe information needed to edit a recipe.
    // It loads the recipe details, ingredients, and nutrition data
    // and returns them inside a RecipeData object.
public RecipeData getRecipeForEdit(int recipeId) throws SQLException {
    ...
    //get all ingredients linked to the recipe
    String ingredientSql = "SELECT ingredients.name, recipe_ingredients.quantity, recipe_ingredients.unit " +
                           "FROM recipe_ingredients " +
                           "JOIN ingredients ON recipe_ingredients.ingredient_id = ingredients.ingredient_id " +
                           "WHERE recipe_ingredients.recipe_id = ?";
    //get all nutrition details for that recipe
    String nutritionSql = "SELECT calories, protein, fat FROM nutrition WHERE recipe_id = ?";
    ...
    try {
        ...
        //return all data inside a RecipeData object
